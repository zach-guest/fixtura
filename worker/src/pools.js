 ...
 *
 * Only straight-up (`su`) scoring is implemented. `pools.mode` is fixed at
 * creation and never changes, so the other modes in the schema are additive
...
async function createPool(request, env, user, origin) {
  ...
  // Only straight-up is implemented. Accepting another mode here would create a
  // pool that can never be scored, and mode is deliberately immutable.
  const mode = String(b.mode || 'su');
  if (mode !== 'su') throw bad("only 'su' (straight up) pools can be created right now");
...
async function weekView(env, ctx, user, poolId, weekArg, origin) {
  ...
  const { results } = await env.DB.prepare(
    `SELECT p.user_id, p.event_id, p.selection_id, u.name
  ...
  for (const r of results) {
    if (r.user_id === user.id) { mine[r.event_id] = r.selection_id; continue; }
...
async function submitPicks(request, env, ctx, user, poolId, origin) {
  ...
  for (const p of b.picks) {
    ...
    seen.add(eventId);
    rows.push({ eventId, selection, kickoff: game.kickoff });
  }

  if (rows.length) {
    await env.DB.batch(rows.map(r => env.DB.prepare(
      `INSERT INTO picks (pool_id, user_id, event_id, week, selection_id, locks_at, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ...
    ).bind(pool.id, user.id, r.eventId, week, r.selection, r.kickoff, t, t, t)));
  }
